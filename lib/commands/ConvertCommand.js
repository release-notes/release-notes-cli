...
class ConvertCommand extends AbstractCommand {
  constructor() {
    ...
    this.converters = {
      ...
      'release-notes': function() {
        this.convert = releaseNotes => ReleaseNotesWriter.dumpReleaseNotes(releaseNotes);
      },
    };
  }
  ...
  runCommand(argv) {
    ...
    if (file.endsWith('.md')) {
      const releaseNotes = this.parseMarkdownChangelog(file);
